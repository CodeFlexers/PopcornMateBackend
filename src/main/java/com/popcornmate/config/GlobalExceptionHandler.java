...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<String> handleTypeMismatch(MethodArgumentTypeMismatchException ex) {
        return ResponseEntity.badRequest().body("요청 파라미터 타입이 잘못되었습니다.");
    }

    // 2. 필수 요청값 누락 (예: @RequestParam(required=true)인데 없음)
    @ExceptionHandler(MissingServletRequestParameterException.class)
    public ResponseEntity<String> handleMissingParam(MissingServletRequestParameterException ex) {
        return ResponseEntity.badRequest().body("필수 요청 파라미터가 누락되었습니다: " + ex.getParameterName());
    }
    ...
    public ResponseEntity<String> handleMessageNotReadable(HttpMessageNotReadableException ex) {
        return ResponseEntity.badRequest().body("요청 형식이 잘못되었습니다. : "+ex.getMessage());
    }
    ...
    public ResponseEntity<String> handleIntegrity(DataIntegrityViolationException ex) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("데이터 제약 조건 위반");
    }

    // 7. 트랜잭션 내부 유효성 검증 오류
    @ExceptionHandler(TransactionSystemException.class)
    public ResponseEntity<String> handleTransaction(TransactionSystemException ex) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("트랜잭션 처리 중 오류 발생");
    }
